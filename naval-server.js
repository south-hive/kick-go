...
class NavalRoom{
  constructor(code,ruleset='modern'){this.ruleset=M.rules(ruleset).id;this.code=code;this.players=[null,null];this.spectators=new Set();this.phase='waiting';this.turn=0;this.match=1;this.revision=0;this.votes=[false,false];this.winner=null;this.lastShot=null;this.shotCount=0;this.lastActivity=Date.now();}
  seat(team,socket,nickname,characterId='default'){characterId=Characters.get(characterId).id;const name=typeof nickname==='string'?[...nickname.replace(/[\u0000-\u001f\u007f]/g,'').trim()].slice(0,16).join(''):'';const player={socket,characterId,token:token(),name:name||`함장 ${team+1}`,lastSeen:Date.now(),ships:[],shots:Array(100).fill(0),sonar:Array(100).fill(false),hitStreak:0,ready:false};this.players[team]=player;L.namePlayer(this,team,nickname);if(this.players.every(Boolean))this.phase=this.lobby?'lobby':'placing';this.revision++;this.lastActivity=Date.now();return player;}
  ...
  fire(team,message){this.check(message);if(!this.connected())throw Error('OPPONENT_OFFLINE');if(this.phase!=='battle'||this.turn!==team)throw Error('NOT_YOUR_TURN');const cell=message.cell;if(!Number.isInteger(cell)||cell<0||cell>=100)throw Error('INVALID_SHOT');const actor=this.players[team],target=this.players[1-team];if(actor.shots[cell])throw Error('ALREADY_SHOT');const ship=target.ships.find(s=>M.cells(s).includes(cell));actor.hitStreak=ship?actor.hitStreak+1:0;actor.shots[cell]=ship?2:1;let sunk=null;if(ship&&M.cells(ship).every(c=>actor.shots[c]>=2)){sunk=ship.id;M.cells(ship).forEach(c=>actor.shots[c]=3);}const detected=!ship&&M.rules(this.ruleset).sonar&&M.sonar(target.ships,cell);actor.sonar[cell]=detected;this.shotCount++;this.lastShot={id:this.shotCount,team,cell,sonar:detected,streak:actor.hitStreak,result:sunk?'sunk':ship?'hit':'miss',sunk,cells:sunk?M.cells(ship):[]};if(target.ships.every(s=>M.cells(s).every(c=>actor.shots[c]>=2))){this.phase='over';this.winner=team;}else this.turn=1-team;this.revision++;this.lastActivity=Date.now();}
  rematch(team,message){if(this.phase!=='over'||message.match!==this.match||!this.connected())throw Error('REMATCH_UNAVAILABLE');this.votes[team]=true;if(this.votes.every(Boolean)){this.match++;this.phase='placing';this.turn=(this.match-1)%2;this.votes=[false,false];this.winner=null;this.lastShot=null;this.shotCount=0;for(const p of this.players){p.ships=[];p.shots=Array(100).fill(0);p.sonar=Array(100).fill(false);p.hitStreak=0;p.ready=false;}}this.revision++;this.lastActivity=Date.now();}
  snapshot(team){const observer=team===null,side=observer?0:team,me=this.players[side],other=this.players[1-side];const sunk=(fleet,shots)=>fleet.filter(s=>M.cells(s).every(c=>shots[c]===3)).map(s=>({...s}));const ownShots=me?[...me.shots]:Array(100).fill(0),incoming=other?[...other.shots]:Array(100).fill(0);return{...L.metadata(this),type:'state',version:1,characters:this.players.map(p=>p?.characterId||'default'),hitStreaks:this.players.map(p=>p?.hitStreak||0),ruleset:this.ruleset,code:this.code,team,role:observer?'spectator':'player',match:this.match,revision:this.revision,phase:this.phase,turn:this.turn,winner:this.winner,names:this.players.map(p=>p?.name||'상대 대기 중'),connected:this.players.map(p=>!!(p?.socket?.readyState===1)),ready:this.players.map(p=>!!p?.ready),votes:[...this.votes],spectators:this.spectators.size,ownShips:me?(observer&&this.phase!=='over'?sunk(me.ships,incoming):me.ships.map(s=>({...s}))):[],ownShots,incomingShots:incoming,ownSonar:me?[...me.sonar]:Array(100).fill(false),incomingSonar:other?[...other.sonar]:Array(100).fill(false),opponentShips:this.phase==='over'&&other?other.ships.map(s=>({...s})):other?sunk(other.ships,ownShots):[],lastShot:this.lastShot?{...this.lastShot,cells:[...this.lastShot.cells]}:null};}
  ...
  wss.on('connection',ws=>{
    ...
    ws.on('message',raw=>{let m;try{
      ...
      if(['create','join','resume','watch'].includes(m.type)){
        ...
        const characterId=['create','join'].includes(m.type)?(m.characterId==='random'?Characters.list()[randomInt(Characters.list().length)].id:Characters.get(m.characterId).id):null;
        if(m.type==='create'){if(rooms.size>=100)throw Error('SERVER_FULL');let code;do{code=randomBytes(6).toString('hex').toUpperCase();}while(rooms.has(code));room=new NavalRoom(code,m.ruleset);L.configure(room,m);rooms.set(code,room);team=0;player=room.seat(team,ws,m.nickname,characterId);}
        else{room=rooms.get(typeof m.code==='string'?m.code.toUpperCase():'');if(!room)throw Error('ROOM_NOT_FOUND');if(m.type==='watch'){if(room.spectators.size>=20)throw Error('SPECTATORS_FULL');room.spectators.add(ws);ws.spectator=true;ws.room=room;send(ws,{type:'joined',version:1,code:room.code,team:null,role:'spectator'});broadcast(room);return;}if(m.type==='join'){if(room.players[1])throw Error('ROOM_FULL');team=1;player=room.seat(team,ws,m.nickname,characterId);}else{const resumed=room.resume(m.token,ws);({team,player}=resumed);if(resumed.previous){resumed.previous.room=null;send(resumed.previous,{type:'ended',reason:'REPLACED'});resumed.previous.close();}}}
        ...
        if(m.type==='prepare'){L.prepare(room,ws.team,m,'placing');broadcast(room);}
        else if(m.type==='ready'){room.ready(ws.team,m);broadcast(room);}
    ...
    ws.on('close',()=>{const room=ws.room;if(ws.spectator&&room){room.spectators.delete(ws);broadcast(room);return;}const p=room?.players[ws.team];if(p?.socket===ws){p.socket=null;p.lastSeen=Date.now();if(room.phase==='lobby')room.lobbyReady=[false,false];broadcast(room);}});
  });

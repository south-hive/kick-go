...
class NavalRoom{
  ...
  seat(team,socket,nickname){const name=typeof nickname==='string'?[...nickname.replace(/[\u0000-\u001f\u007f]/g,'').trim()].slice(0,16).join(''):'';const player={socket,token:token(),name:name||`함장 ${team+1}`,lastSeen:Date.now(),ships:[],shots:Array(100).fill(0),ready:false};this.players[team]=player;L.namePlayer(this,team,nickname);if(this.players.every(Boolean))this.phase=this.lobby?'lobby':'placing';this.revision++;this.lastActivity=Date.now();return player;}
  ...
  wss.on('connection',ws=>{
    ...
    ws.on('message',raw=>{let m;try{
      ...
      if(['create','join','resume','watch'].includes(m.type)){
        ...
        if(m.type==='create'){if(rooms.size>=100)throw Error('SERVER_FULL');let code;do{code=randomBytes(6).toString('hex').toUpperCase();}while(rooms.has(code));room=new NavalRoom(code);L.configure(room,m);rooms.set(code,room);team=0;player=room.seat(team,ws,m.nickname);}

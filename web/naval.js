(() => {
  ...
  const reaction=NavalReactions.install($('naval-reaction'),{counts:$('naval-hit-count')});
  for(const preset of GameCharacters.list())$('naval-character').add(new Option(preset.name,preset.id));
  try{const saved=localStorage.getItem('arcade-character');if([...$('naval-character').options].some(o=>o.value===saved))$('naval-character').value=saved;}catch{}
  $('naval-character').onchange=()=>{try{localStorage.setItem('arcade-character',$('naval-character').value);}catch{}};
  ...
  function onState(next){
    ...
    if(draftKey!==key){reaction.reset();text('naval-effect','');draftKey=key;draft=[];selected='carrier';vertical=false;lastEvent=`${next.match}:${next.lastShot?.id||0}`;try{const saved=JSON.parse(sessionStorage.getItem('naval-draft:v1'));if(saved?.key===key)draft=M.validate(saved.ships,false);}catch{}}
  ...
  function render(){
    if(!net.session&&!net.state){state=null;reaction.reset();}
    ...
    const rules=M.rules(state?.ruleset||$('naval-ruleset').value);
    $('naval-sonar-help').hidden=!rules.sonar;$('naval-sonar-legend').hidden=!rules.sonar;text('naval-room-ruleset',rules.name);
    if(!state)return;
    ...
    paint('own',ownShips,state.incomingShots,editable(),state.incomingSonar);paint('enemy',state.opponentShips,state.ownShots,canFire(),state.ownSonar);
  ...
  function paint(which,ships,shots,enabled,sonar=[]){const occupied=new Map();for(const ship of ships)for(const cell of M.cells(ship))occupied.set(cell,ship);boards[which].forEach((button,cell)=>{const ship=occupied.get(cell),shot=shots[cell],detected=shot===1&&sonar[cell];button.className='sea-cell'+(ship?' ship'+(ship.vertical?' vertical':''):'')+(detected?' sonar':'')+(shot===1?' miss':shot===2?' hit':shot===3?' sunk':'');button.textContent=detected?'?':shot===1?'·':shot===2?'✦':shot===3?'✕':'';button.disabled=!enabled||(which==='enemy'&&!!shot);button.setAttribute('aria-label',`${M.coordinate(cell)} ${detected?'빗나감 · 중심 3×3에 함선 감지':shot===1?'빗나감':shot===2?'명중':shot===3?'격침':ship?M.FLEET.find(s=>s.id===ship.id).name:'미확인'}`);});}
  ...
  function enter(type){const code=$('naval-code').value.trim().toUpperCase();if(type!=='create'&&!/^[A-F0-9]{12}$/.test(code)){text('naval-status','12자리 방 코드를 입력하세요.');$('naval-code').focus();return;}state=null;net.begin(type,code,$('naval-nickname').value,{ruleset:$('naval-ruleset').value,characterId:$('naval-character').value});}
  $('naval-ruleset').onchange=render;
  ...
  $('naval-rematch').onclick=()=>net.send('rematch');$('naval-reconnect').onclick=()=>{const saved=net.session||net.saved();if(saved)net.restore(saved);else if(net.action)net.begin(net.action.type,net.action.code,net.action.nickname,net.action);};
